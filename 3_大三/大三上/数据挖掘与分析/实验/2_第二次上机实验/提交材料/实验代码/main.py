...
plt.savefig(r"..\实验结果\PCA散点图.png")
plt.savefig(r"..\实验报告\LaTeX源代码\figures\fig1.png")
sc.pl.pca_variance_ratio(adata, log=True, show=False) # 计算单个PC对数据总方差的贡献
plt.savefig(r"..\实验结果\单个PC对数据总方差的贡献.png")
...
plt.savefig(r"..\实验结果\leiden聚类邻域图.png")
plt.savefig(r"..\实验报告\LaTeX源代码\figures\fig3.png")
plt.show()
plt.close()
adata.write(r"..\实验结果\聚类得分矩阵（未标记细胞类型）.h5ad") # 保存结果
labels_pred=np.array((sc.read_h5ad(r"..\实验结果\聚类得分矩阵（未标记细胞类型）.h5ad")).obs)
(pd.DataFrame(labels_pred)).to_csv(r"..\实验结果\聚类标签.csv", index=False)
...
def KM(L1,L2): # KM（Kuhn-Munkres）匈牙利算法
...
def purity_score(y_true, y_pred): # 定义purity（纯度）计算函数
...
plt.savefig(r"..\实验结果\每个簇中高度差异基因排名.png")
plt.savefig(r"..\实验报告\LaTeX源代码\figures\fig4.png")
plt.show()
plt.close()
...
plt.savefig(r"..\实验结果\聚类结果散点图（标记细胞类型）.png")
plt.savefig(r"..\实验报告\LaTeX源代码\figures\fig5.png")
plt.show()
plt.close()
sc.pl.dotplot(adata, marker_genes, groupby='leiden', show=False); # 绘制标记基因气泡图
plt.savefig(r"..\实验结果\标记基因气泡图.png")
plt.savefig(r"..\实验报告\LaTeX源代码\figures\fig6.png")
plt.show()
plt.close()
sc.pl.stacked_violin(adata, marker_genes, groupby='leiden', rotation=90, show=False); # 绘制标记基因小提琴图 
plt.savefig(r"..\实验结果\标记基因小提琴图.png")
plt.savefig(r"..\实验报告\LaTeX源代码\figures\fig7.png")
plt.show()
plt.close()
adata.write(r"..\实验结果\聚类得分矩阵（标记细胞类型）.h5ad") # 保存结果
