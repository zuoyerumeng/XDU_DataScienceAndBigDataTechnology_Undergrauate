...
# 一些基本量的计算
mtx=adata.X  # 获取mtx文件（scipy.sparse._csr.csr_matrix对象）
...
plt.savefig(r"统计分析的可视化结果（小提琴图）.png")
plt.show()
plt.close()
...
plt.savefig(r"高度可变基因表达情况散点图.png")
plt.show()
plt.close()

    # 计算并可视化所有细胞中表达占比最高的基因
sc.pl.highest_expr_genes(adata, n_top=20, show=False)
plt.savefig(r"所有细胞中表达占比最高的基因.png")
plt.show()
plt.close()
...
for i in range(len(mtx.indptr)-1):
    ...
    # 计算所有基因对两两之间的皮尔森相关系数
max_memory=500
expression_mtx_transpose=np.transpose(expression_mtx)
expression_df=pd.DataFrame(np.transpose(expression_mtx_transpose[:max_memory]))
pearson_genes=expression_df.corr(method="pearson")
pearson_genes.to_excel(r"基因对之间的皮尔森相关系数.xlsx")        
        # 将结果存为由（节点，节点，权重）格式构成的网络表格
network_genes_np=np.array([[]])
for i in range(max_memory):
    for j in range(i):
            # 按皮尔森相关系数大于0.7的条件，过滤出相似性较强的的节点之间的边
        if pearson_genes[i][j]>0.7: 
            np.append(network_genes_np,[i,j,pearson_genes[i][j]])
network_genes_df=pd.DataFrame(network_genes_np)
network_genes_df.to_excel(r"基因对之间的皮尔森相关系数网络.xlsx")

    # 用余弦相似度计算细胞之间的相似性
cosine_similarity_np=cosine_similarity(expression_mtx[:max_memory])
cosine_similarity_pd=pd.DataFrame(cosine_similarity_np) # 把npy格式文件转换为DataFrame数据类型
cosine_similarity_pd.to_excel(r"细胞之间的相似度.xlsx") # 将结果存为Excel表格     
        # 将结果存为由（节点，节点，权重）格式构成的网络表格
network_barcodes_np=np.array([[]])
for i in range(max_memory):
    for j in range(i):
            # 按余弦相似度大于0.95的条件，过滤出相似性较强的的节点之间的边
        if cosine_similarity_np[i][j]>0.7: 
            np.append(network_barcodes_np,[i,j,cosine_similarity_np[i][j]])
network_barcodes_df=pd.DataFrame(network_barcodes_np)
network_barcodes_df.to_excel(r"细胞之间的相似度网络.xlsx")
